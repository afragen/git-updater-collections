...
class Repo_List_Table extends \WP_List_Table {
	...
	public function column_uri( $item ) {
		...
		// Build row actions.
		$actions = [
			// 'edit'   => sprintf( '<a href="%s&action=%s&uri=%s">Edit</a>', $location, 'edit', $item['ID'] ),
			'delete' => sprintf( '<a href="%s&action=%s&uri=%s">Delete</a>', wp_nonce_url( $location, 'delete_row_item', '_wpnonce_row_action_delete' ), 'delete_row_item', $item['ID'] ),
		];
	...
	public function process_bulk_action() {
		...
		$uris = isset( $_REQUEST['uri'] ) ? sanitize_key( wp_unslash( $_REQUEST['uri'] ) ) : null;
		$uris = is_array( $uris ) ? $uris : (array) $uris;
		foreach ( $uris as $uri ) {
